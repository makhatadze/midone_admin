<?php
   ...
   class Ticket extends Model
   {
       ...
       public function getApproveDepartments()
       {
           ...
           $additionalDepartmentsModel = $this->additionalDepartments;

           // Return default departments if we dont have any additional ones.
           if (!$additionalDepartmentsModel) {
               return $data;
           }

           $additionalDepartmentIds = unserialize($additionalDepartmentsModel->additional_departments);
            
           // Get Unique Arrays
           $leftDeparmentIds = array_diff($additionalDepartmentIds, array_column($data,'department_id'));

           // add additional departments
           foreach ($leftDeparmentIds as $depId) {
               $data[] = $this->getApproveArray($depId);
           }
       ...
       {
           return $this->hasOne(TicketDepartments::class, 'ticket_id', 'id');
       }
