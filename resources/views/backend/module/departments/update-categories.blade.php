...
@section('subhead')
    <title>Insite - Update Categories</title>
@endsection
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-8">
            <div class="intro-y box p-5">
                {!! Form::open(['route' => ['categoriesUpdate',$category->id],'method' =>'post',]) !!}
                <div class="sm:grid grid-cols-1 gap-1 mb-4">
                    <div class="relative mt-4 {{ $errors->has('category_name') ? ' has-error' : '' }}">
                        ...
                        {{ Form::text('category_name', $category->name, ['class' => 'input w-full border mt-2 col-span-2', 'no','placeholder'=>'Category Name...','id' => 'department_name']) }}
                ...
                <div class="sm:grid grid-cols-1 gap-1 mb-4">
                    <div class="relative mt-4 {{ $errors->has('department_group') ? ' has-error' : '' }}">
                        ...
                        <select name="department_group[]" data-placeholder="Select Department Group"
                            ...
                            @if($departments)
                                @foreach($departments as $department)
                                    <option value="{{$department->id}}" {{in_array($department->id,array_column($departmentGroup,'id')) ? 'selected' : ''}}>{{$department->name}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
